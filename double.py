...
class double:
    def __init__(self,url,type_url,login,Parsing,label,write,clean,write_SQL,cpu_count,sem):
        self.files,self.url,self.htmls = [],'',[]
        self.title = label.keys()
        #self.sem = asyncio.Semaphore(sem)
        if cpu_count == 0:cpu_count = multiprocessing.cpu_count()//2
    ...
    def run(self):
        ...
        tasks = [self.handle_tasks(task_id,q,) for task_id in range(self.cpu_count)]
        result = loop.run_until_complete(asyncio.wait(tasks))
        loop.close()
        return result
    def parse_results(self):
        if len(self.Parsing):
            parm_texts = self.main_parse_html()
            print(parm_texts)
    ...
    def multi_parse_html(self,text):
        '''
        爬虫处理阶段
        ...
        if self.Parsing.lower() == 're':
            ...
            for title in self.title:
                ...
                label_text = re.findall(r'{}'.format(label[0]),text,'re.S')
    ...
    def main_parse_html(self):
        ...
        for html in self.htmls:
            parm_texts = p.apply_async(self.multi_parse_html,args=(html,))
        p.close()
        p.join()
        return parm_texts
    ...
    def write_json(self,texts):
        ...
        import json
        for text in texts:
            text = json.dumps(text, indent=4, cls=MyEncoder)
            with open(self.write, 'a', encoding='utf-8') as f:
                f.write("{\n"+text+"\n}")
