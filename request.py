...
class parse:
    def __init__(
        ...
        next_url='',page=[False],clean=False,write_SQL={} \
            ):
        '''
        爬虫程序执行
    ...
    def judge(self):
        '''  
        检查传参格式是否正常
        :param first_url,type_url,login,Parsing,label,write,next_url,page,clean,write_SQL
        :return True or False
        '''
        if not isinstance(self.first_url, str):raise ValueError('url is str')
        if not isinstance(self.type_url, str):raise ValueError('type_url is str')
        if not isinstance(self.login, str):raise ValueError('login is str')
        if not isinstance(self.Parsing, str):raise ValueError('Parsing is str')
        if not isinstance(self.label, dict):raise ValueError('label is dict')
        if not isinstance(self.write, str):raise ValueError('write is str')
        if not isinstance(self.next_url, str):raise ValueError('next_url is str')
        if not isinstance(self.page, list):raise ValueError('page is list')
        if not isinstance(self.clean, bool):raise ValueError('clean is bool')
        if not isinstance(self.write_SQL, dict):raise ValueError('write_SQL is dict')
    ...
    def run(self,*url):
        ...
        '''
        if self.page:
            if len(self.page) == 2:name = self.page[1]
            else:name = 'page.txt'
            if os.path.exists(name):
                with open(name,'r') as f:
                    url = f.read()
            if url:self.text = self.request(url)
            else:self.text = self.request(self.first_url)
        else:
            if url:self.text = self.request(url)
            else:self.text = self.request(self.first_url)
        if len(self.Parsing):
    ...
    def Handling_saving(self):
        ...
        if len(self.write):
            ...
            if len(self.url):
                if self.url[0:4] == 'http':self.run(self.url)
                else:self.run(self.first_url + self.url)
    def request(self,url,*proxy,*params):
        '''
        爬虫初期访问阶段
        ...
        try:
            if self.login:
                ...
                if os.path.isfile(self.login+'_session.pkl'):
                    ...
                    f.close()
                if params:html = self.session.get(url,params = params)
                else:html = self.session.get(url)
            else:
                if params:
                    if proxy:html = requests.get(url,headers = self.headers,proxies = proxy,params = params)
                    else:html = requests.get(url,headers = self.headers,params = params)
                else:
            ...
            self.times += 1
            self.ip_mode = ip_proxy.xici_proxy()
    ...
    def parsing(self):
        ...
        if self.Parsing.lower() == 're':
            ...
            if self.next_url:
                try:self.url = re.findall(r'{}'.format(self.next_url),self.text,'re.S')[0]
                except:self.url = ''
            if self.page[0]:
                if self.url[0:4] == 'http':self.write_page(self.url)
                else:self.write_page(self.first_url + self.url)
        ...
        if self.Parsing.lower() == 'xpath':
            ...
            for title in self.title:
                ...
                texts[title] = label_text
            if self.next_url:
                try:self.url = dom.xpath(r'{}'.format(self.next_url))[0]
                except:self.url = ''
            if self.page[0]:
                    if self.url[0:4] == 'http':self.write_page(self.url)
                    else:self.write_page(self.first_url + self.url)
        ...
        if self.Parsing.lower() == 'bs4':
            ...
            for title in self.title:
                ...
                texts[title] = label_text
            if self.next_url:
                try:self.url = bs.select(self.next_url)[0].get_text()
                except:self.url = ''
            if self.page[0]:
                if self.url[0:4] == 'http':self.write_page(self.url)
                else:self.write_page(self.first_url + self.url)
    ...
    def write_ROM(self,texts):
        ...
        if self.write.split('.')[-1] == 'txt':
            ...
            print('[warning] : Incorrect access')
    ...
    def write_pkl(self,texts):
        ...
        for text in texts:
            with open(self.write, 'ab') as f:
                pickle.dump(text, f)
    def write_page(self,url):
        '''
        断续笔记函数
        url写入记事本内部,下次爬虫直接调取记事本中继续爬虫
        :param url:此次爬虫进行下的url网址链接
        :return txt:保存文件
        '''
        if len(self.page) == 2:name = self.page[1]
        else:name = 'page.txt'
